...
while epoch < numEpochs
    ...
    while hasdata(fpm_cube)
        ...
        % if mod(iteration,50) == 0
            % psf = fftshift(fft2(wavefront2));
            % [psf,shift_x,shift_y] = adjust_psf_center(psf);
            % wavefront2 = ifft2(ifftshift(psf)) .* CTF_object0;
            
            % wavefront1 = circshift(circshift(wavefront1,round(shift_x),2),round(shift_y),1);
            % this_mom1 = fftshift(fft2(optimizer_w2.mom1));
            % this_mom1 = circshift(circshift(this_mom1,round(shift_x),2),round(shift_y),1);
            % optimizer_w2.mom1 = ifft2(ifftshift(this_mom1)) .* CTF_object0;

            % this_mom1 = fftshift(fft2(optimizer_w2.squave));
            % this_mom1 = circshift(circshift(this_mom1,round(shift_x),2),round(shift_y),1);
            % optimizer_w2.squave = ifft2(ifftshift(this_mom1)) .* CTF_object0;
        % end
    end

    
    % psf = fftshift(fft2(wavefront2));
    % [psf,~,~] = adjust_psf_center(psf);
    % wavefront2 = ifft2(ifftshift(psf)) .* CTF_object0;
...
save('FD-FPM_abb.mat','wavefront2','CTF_object0');
end
