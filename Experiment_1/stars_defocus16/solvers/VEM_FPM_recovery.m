...
oI = (imresize(mean(I_camera(:,:,2:9),3),m_factor * magg)); 
...
oI = data_formated(imresize(mean(I_camera(:,:,2:9),3),m_factor * magg)); 
...
while epoch < numEpochs
    ...
    oI = medfilt2(real(oI)) + 1i * medfilt2(imag(oI));
    % oI = complex_TV(oI,0.001,'isotropic');
    ...
    wavefront2 = deconv_data.oth./(deconv_data.oto + 0.01) .* CTF_object0;
...
out = wavefront1;
save('VEM-FPM_abb.mat','wavefront2')
end
...
% [loss,dm] = retinex_loss(dY_obs,abs(latent_z),0.02,fspecial('gaussian',5,1));
...
function [out,o,err] = M_step(dec_o,dtd,oR,type)
    ...
    switch type
        case 'normal'
            ...
            fenmu = dec_o.hth + 1;
        case 'retinex'
            fenzi = dec_o.hto .* dtd + fftshift(fft2(0.01*oR)); %tv_max * (v - u) + 
            fenmu = dec_o.hth .* dtd  + 0.01; %+ tv_max
        otherwise
