...
for shifting_group = 1:length(fqc_shift)
    for amplitude_group = 1:length(amplitude)
        for testing_group = 1:10

%% preparing reconstruction data for WARE engine
img1 = double(imread('source_image/5.3.02.tiff'))/255;
...
ground_truth_u = u;
save ground_truth ground_truth_u
% global aberrations
...
% this_name = ['outdata_amplitude=',num2str(amplitude_group),'_group=',num2str(testing_group)];
save(['noise_test_20250212\',this_name,'.mat'],'saved_data');
% saved_data.I_low = this_datacube;
% save('reducedData/data_reduced.mat', 'saved_data')
...
lambda = 0.62863; % wavelength um
k = 2*pi/lambda;
NA = 0.25; %nurmical aperture
...
%% LED properties
h_LED = 50; % distance between LED matrix and sample
...
% img = getframe(gcf);
% 
% imwrite(img.cdata,['saved_data//LED_pos',num2str(d),'_num',num2str(num),'_.png'])
end
